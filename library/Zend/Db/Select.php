...
class Zend_Db_Select
{
    ...
    public function minus($select = array(), $type = self::SQL_MINUS)
    {
        if (!is_array($select)) {
            require_once 'Zend/Db/Select/Exception.php';
            throw new Zend_Db_Select_Exception(
                "minus() only accepts an array of Zend_Db_Select instances of sql query strings."
            );
        }
        
        foreach ($select as $target) {
            $this->_parts[self::MINUS][] = array($target, $type);
        }
        return $this;        
    }
...
/*                 require_once 'Zend/Db/Select/Exception.php';
    ...
    /**
     * Renderiza MINUS query
     * 
     * @param string $sql SQL query
     * @return string
     */
    protected function _renderMinus($sql)
    {
        if ($this->_parts[self::MINUS]) {
            $parts = count($this->_parts[self::MINUS]);
            
            foreach ($this->_parts[self::MINUS] as $cnt => $union) {
                list($target, $type) = $union;
                if ($target instanceof Zend_Db_Select) {
                    $target = $target->assemble();
                }
