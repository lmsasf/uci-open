...
class ServiceBroker implements IServiceBroker
{
    ...
    public function getService($type, array $options = array())
    {
        ...
        if (!is_string($type) || !is_subclass_of($type, '\\Asf\\Service\\Service')) {
            throw new InvalidArgumentException('$type must be a classname of a class inheriting from \\Asf\\Service\\Service ("' . $type . '" given)');
        }
    ...
    /**
     * Crea una instancia local del servicio, usa siempre las opciones pasadas como argumentos
