...
================================================== */
    ...
    $('.middleNav ul li').click(function () {
        //$('.middleNav ul li').find('ul').slideUp(100);
...
//    $('#breadcrumbs').xBreadcrumbs();
    ...
    $("select, input:checkbox, input:radio, input:file").not('.exclude').uniform();
    




    //        $(".openMenu").find('a').toggle(
    //            function(e){
    //                e.preventDefault();
    //                $("#leftSide").hide();
    //                $("body").removeClass('background');
    //                $(this).find('img').attr('src', 'images/next.png').parent().parent().css({'margin-left':'0em'});
    //            },
    //            function(e){
    //                e.preventDefault();
    //                $("#leftSide").show();
    //                $("body").addClass('background');
    //                $(this).find('img').attr('src', 'images/prev.png').parent().parent().css({'margin-left':'18.1em'});
    //            }
    //        );
            
    //$(".openMenu").find('a').trigger('click').parent().hide();
            
    $(".collapse").hide().parent().css({
        'cursor': 'pointer'
    });
    $(".title").click(
        function(){
...
function openHelp(file){
	window.open('/pcg/zpae/help/HTML/'+file+'.htm',"");
}

/**
 *  SOPORTE PARA TRADUCCIONES DE JAVASCRIPT 
 *  
 *  TODO Mejorar esta función cuando sea posible 
 */ 
function _(s) {
	// se asume que se incluye un archivo en donde esta declarado el objeto l10n, de lo contrario falla
...
/**
 * Función que ayuda a determinar si el objeto esta vacío
...
function displayMessage( tipo, mensaje, prefrase, redirect ){
    $("html, body").animate({scrollTop:"0px"});  
    console.log($("html, body"));
	$("#mensajes").html('');
	if(redirect != '' ) {
		mensaje = mensaje + '  redirecting...';
	}
	$('#mensajes').message({ type: tipo,  frase: mensaje, prefrase: prefrase  });
	window.setTimeout(function() {
		if(redirect != '' ) {
			window.location = redirect;
		} else {
			$("#mensajes").fadeOut('slow', function() {
				$(this).html('');
				$(this).show();
			});
		}
...
function displayMessagenew( tipo, mensaje, prefrase, redirect ){ 	
	var form = $('<form>').attr({
		 name:'editsaveform', 
		 method: 'post', 
		 action: redirect
	...
	form.appendTo($('body'));
	form.submit();	
	tipo = null;mensaje = null;prefrase = null; redirect = null;$("#mensajes").html('');
}
